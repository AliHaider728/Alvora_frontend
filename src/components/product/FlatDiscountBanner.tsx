...
export const FlatDiscountBanner: React.FC<FlatDiscountBannerProps> = ({ flatDiscount, selectedQuantity, isOverridden = false }) => {
  ...
  return (
    <div className={`mt-5 w-full overflow-hidden rounded-2xl border ${isApplied ? 'border-[#C48B80] bg-[#C48B80]' : 'border-slate-200 bg-slate-50'}`}>
      <div className="flex items-center gap-3 px-4 py-3 sm:px-5">
        <div className={`flex shrink-0 items-center justify-center rounded-xl p-2 ${isApplied ? 'bg-[#C48B80] text-[#C48B80]' : 'bg-slate-100 text-slate-500'}`}>
          <BadgePercent className="h-5 w-5" />
        </div>
        <div className="flex min-w-0 flex-1 flex-col">
          <span className={`truncate text-sm font-black ${isApplied ? 'text-[#C48B80]' : 'text-slate-700'}`}>
            {displayLabel}
          </span>
          <span className={`truncate text-xs font-bold ${isApplied ? 'text-[#C48B80]' : 'text-slate-500'}`}>
            {isApplied
      ...
      {isApplied && (
        <div className="bg-[#C48B80] px-4 py-2 text-center text-[10px] font-black uppercase tracking-wider text-white">
