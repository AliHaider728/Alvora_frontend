  ...
  return (
    <section className="rounded-3xl border border-[#E7D9D0] bg-white p-5 shadow-sm sm:p-6">
          ...
          return (
            <article key={block.id} className="overflow-hidden rounded-2xl border border-[#E7D9D0] bg-[#FAF6F2]">
              ...
              {!isCollapsed && <div className="space-y-4 p-4">
                {block.type === 'richText' && <>
                  <label className="block text-xs font-bold text-[#1A1A1A]/70">Optional heading<input maxLength={140} value={block.heading || ''} onChange={event => update(block.id, { heading: event.target.value })} className="mt-1 w-full rounded-xl border border-[#E7D9D0] bg-white px-3 py-2.5 text-sm" /></label>
                ...
                {['heading', 'featureCards', 'iconText', 'benefitsList', 'whatsIncluded', 'recommendedAge', 'giftBadges', 'spacer', 'ctaBanner', 'imageText', 'fullWidthImage', 'gallery'].includes(block.type) && <>
                  <label className="block text-xs font-bold text-[#1A1A1A]/70">Heading<input maxLength={140} value={block.heading || ''} onChange={event => update(block.id, { heading: event.target.value })} className="mt-1 w-full rounded-xl border border-[#E7D9D0] bg-white px-3 py-2.5 text-sm" placeholder="Block heading" /></label>
                  <label className="block text-xs font-bold text-[#1A1A1A]/70 mt-3">Content (optional)<textarea rows={4} maxLength={2000} value={block.content || ''} onChange={event => update(block.id, { content: event.target.value })} className="mt-1 w-full rounded-xl border border-[#E7D9D0] bg-white px-3 py-2.5 text-sm" placeholder="Block text content" /></label>
                </>}
                {['image', 'imageText', 'fullWidthImage', 'gallery', 'ctaBanner'].includes(block.type) && <>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2 mt-3">
                    <label className="text-xs font-bold text-[#1A1A1A]/70">Alt text *<input maxLength={180} value={block.image?.alt || ''} onChange={event => block.image && update(block.id, { image: { ...block.image, alt: event.target.value } })} className="mt-1 w-full rounded-xl border border-[#E7D9D0] bg-white px-3 py-2.5 text-sm" /></label>
                    <label className="text-xs font-bold text-[#1A1A1A]/70">Caption<input maxLength={300} value={block.image?.caption || ''} onChange={event => block.image && update(block.id, { image: { ...block.image, caption: event.target.value } })} className="mt-1 w-full rounded-xl border border-[#E7D9D0] bg-white px-3 py-2.5 text-sm" /></label>
                  </div>
                </>}
                {block.type === 'html' && isSuperAdmin && (
                  <div>
                    <label className="block text-xs font-bold text-[#1A1A1A]/70">Custom HTML<textarea rows={10} maxLength={30000} spellCheck={false} value={block.content || ''} onChange={event => update(block.id, { content: event.target.value })} className="mt-1 w-full rounded-xl border border-[#E7D9D0] bg-slate-950 p-3 font-mono text-xs text-emerald-200" /></label>
                    <p className="mt-2 text-[11px] font-semibold text-[#C48B80]">Paste only the product-section HTML. Remove &lt;html&gt;, &lt;head&gt;, &lt;body&gt;, and &lt;style&gt; tags.</p>
                  </div>
                )}
                ...
                {block.type === 'divider' && <div className="py-6"><hr className="border-[#E7D9D0]" /></div>}
                {block.type !== 'divider' && <div className="grid gap-3 sm:grid-cols-2">
                  <label className="text-xs font-bold text-[#1A1A1A]/70">Width<select value={block.settings?.width || 'full'} onChange={event => update(block.id, { settings: { ...block.settings, width: event.target.value as 'full' | 'large' | 'medium' } })} className="mt-1 w-full rounded-xl border border-[#E7D9D0] bg-white px-3 py-2.5"><option value="full">Full</option><option value="large">Large</option><option value="medium">Medium</option></select></label>
                  <label className="text-xs font-bold text-[#1A1A1A]/70">Alignment<select value={block.settings?.alignment || 'center'} onChange={event => update(block.id, { settings: { ...block.settings, alignment: event.target.value as 'left' | 'center' | 'right' } })} className="mt-1 w-full rounded-xl border border-[#E7D9D0] bg-white px-3 py-2.5"><option value="left">Left</option><option value="center">Center</option><option value="right">Right</option></select></label>
                </div>}
              </div>}
            </article>
          );
        })}
        {sorted.length === 0 && <div className="rounded-2xl border border-dashed border-[#E7D9D0] p-8 text-center text-xs font-medium text-[#1A1A1A]/40">No custom product-page blocks yet.</div>}
      </div>
      ...
      {isSuperAdmin && <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-4">
        ...
        <span className="block text-right text-[10px] font-bold text-[#A86249]">{customCss.length}/10000</span>
      </div>}

      {sorted.length > 0 && <div className="mt-6 rounded-2xl border border-[#E7D9D0] bg-white p-4">
        <h3 className="mb-3 text-xs font-black uppercase tracking-wide text-[#1A1A1A]/50">Isolated preview</h3>
        <iframe title="Product content preview" sandbox="" className="min-h-72 w-full rounded-xl border border-[#E7D9D0]" srcDoc={makePreviewDocument(sorted, isSuperAdmin ? customCss : '')} />
      </div>}
    </section>
  );
};
