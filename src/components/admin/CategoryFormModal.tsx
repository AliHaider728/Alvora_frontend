  ...
  return <div className="fixed inset-0 z-[105] flex items-center justify-center overflow-y-auto bg-slate-950/60 p-3 sm:p-5" onMouseDown={event => { if (event.target === event.currentTarget) void safeClose(); }}>
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="category-form-title" className={`my-auto max-h-[calc(100vh-1.5rem)] w-full overflow-y-auto rounded-3xl bg-white shadow-2xl ${compact ? 'max-w-2xl' : 'max-w-4xl'}`}>
      ...
      <form onSubmit={submit} className="grid gap-5 p-5 md:grid-cols-2">
        <label className="text-xs font-bold text-[#1A1A1A]/80">Category Name *<input ref={firstRef} value={name} maxLength={80} onChange={e => { setName(e.target.value); if (!slugEdited) setSlug(slugify(e.target.value)); }} className={input} />{errors.name && <span className="mt-1 block text-[#A86249]">{errors.name}</span>}</label>
        <label className="text-xs font-bold text-[#1A1A1A]/80">Slug *<input value={slug} maxLength={90} onChange={e => { setSlug(e.target.value); setSlugEdited(true); }} className={input} />{errors.slug && <span className="mt-1 block text-[#A86249]">{errors.slug}</span>}</label>
        <label className="text-xs font-bold text-[#1A1A1A]/80 md:col-span-2">Short Description<textarea rows={2} maxLength={240} value={shortDescription} onChange={e => setShortDescription(e.target.value)} className={input} /></label>
        {!compact && <label className="text-xs font-bold text-[#1A1A1A]/80 md:col-span-2">Description<textarea rows={3} maxLength={1200} value={description} onChange={e => setDescription(e.target.value)} className={input} /></label>}
        ...
        <label className="text-xs font-bold text-[#1A1A1A]/80">Navigation Label<input value={navigationLabel} maxLength={60} onChange={e => setNavigationLabel(e.target.value)} className={input} placeholder="Defaults to category name" /></label>
        <label className="text-xs font-bold text-[#1A1A1A]/80">Display Order<input type="number" min="0" value={displayOrder} onChange={e => setDisplayOrder(Number(e.target.value))} className={input} /></label>
        {!compact && <label className="text-xs font-bold text-[#1A1A1A]/80">Parent Category<select value={parentCategoryId} onChange={e => setParentCategoryId(e.target.value)} className={input}><option value="">No parent</option>{categories.filter(item => item.id !== category?.id && !item.parentCategoryId).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}
        <div className="grid grid-cols-2 gap-3 md:col-span-2 sm:grid-cols-3">
          {[[isActive, setIsActive, 'Active'], [showInNavigation, setShowInNavigation, 'In Navigation'], [isFeatured, setIsFeatured, 'Featured'], [desktopVisible, setDesktopVisible, 'Desktop'], [mobileVisible, setMobileVisible, 'Mobile']] .map(([checked, setter, label]) => <label key={String(label)} className="flex items-center gap-2 rounded-xl border border-[#E7D9D0] p-3 text-xs font-bold text-[#1A1A1A]/80"><input type="checkbox" checked={checked as boolean} onChange={e => (setter as React.Dispatch<React.SetStateAction<boolean>>)(e.target.checked)} />{String(label)}</label>)}
        </div>
