...
class X {
  ...
  #initRenderer() {
    ...
    const userOptions = { ...(this.#config.rendererOptions ?? {}) } as any;
    ...
    const glContext =
      (this.canvas.getContext('webgl2', contextOptions) as WebGL2RenderingContext | null) ||
      (this.canvas.getContext('webgl', contextOptions) as WebGLRenderingContext | null);

    if (!glContext) {
      throw new Error('Three: Unable to create WebGL context');
    }
