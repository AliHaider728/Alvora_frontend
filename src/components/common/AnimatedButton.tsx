...
export const AnimatedButton = React.forwardRef<HTMLButtonElement | HTMLAnchorElement, AnimatedButtonProps>(
  ({ href, variant = 'primary', size = 'md', hideLeaves = false, className = '', children, ...props }, ref) => {
    ...
    if (variant === 'primary') {
      ...
      svgColor = "text-white/20 drop-shadow-md";
    } else if (variant === 'white') {
      baseClass += "bg-white text-[#9C4122] border border-transparent rounded-xl hover:bg-[#1A1A1A] hover:text-white overflow-hidden ";
      svgColor = "text-[#C87355]/30 group-hover:text-white/20 transition-colors";
    } else if (variant === 'outline') {
      baseClass += "bg-transparent text-[#181818] border-2 border-[#181818] rounded-[8px] hover:bg-[#181818] hover:text-white overflow-hidden ";
      svgColor = "text-current/20";
    } else if (variant === 'secondary') {
      baseClass += "bg-[#F5EDE4] text-[#1A1A1A] hover:bg-[#E8DCCB] rounded-[8px] border border-transparent overflow-hidden ";
      svgColor = "text-[#C48B80]/40";
    } else if (variant === 'danger') {
      baseClass += "bg-red-50 text-red-600 border border-red-200 rounded-[8px] hover:bg-red-100 overflow-hidden ";
      svgColor = "text-red-500/30";
    }

    const finalClassName = `${baseClass} ${className}`.trim();

    /* ================================================================
       SVG LEAF POSITIONS — exact from original styled-components:
       .icon-1 → top: 0, right: 0,     width: 25px  (RIGHT)
       .icon-2 → top: 0, left: 25px,   width: 12px  (LEFT inner)
       .icon-3 → top: 0, left: 0,      width: 18px  (LEFT corner)
       ================================================================ */
    const SVGs = (
      <>
        {/* icon-1: RIGHT — top:0 right:0 w:25px */}
        <div className={`anim-slay-1 absolute top-0 right-0 w-[18px] sm:w-[25px] pointer-events-none will-change-transform drop-shadow-[2px_2px_3px_rgba(0,0,0,0.3)] ${svgColor}`}>
          <svg viewBox="0 0 26.3 65.33" xmlns="http://www.w3.org/2000/svg" fill="currentColor" style={{shapeRendering: 'geometricPrecision', fillRule: 'evenodd', clipRule: 'evenodd'}}>
        ...
        {/* icon-2: LEFT inner — top:0 left:25px w:12px — CORRECT SVG viewBox 11.67x37.63 */}
        <div className={`anim-slay-2 absolute top-0 left-[8px] sm:left-[12px] w-[10px] sm:w-[12px] pointer-events-none will-change-transform drop-shadow-[2px_2px_3px_rgba(0,0,0,0.5)] ${svgColor}`}>
          <svg viewBox="0 0 11.67 37.63" xmlns="http://www.w3.org/2000/svg" fill="currentColor" style={{shapeRendering: 'geometricPrecision', fillRule: 'evenodd', clipRule: 'evenodd'}}>
        ...
        {/* icon-3: LEFT corner — top:0 left:0 w:18px */}
        <div className={`anim-slay-3 absolute top-0 left-0 w-[13px] sm:w-[18px] pointer-events-none will-change-transform drop-shadow-[2px_2px_3px_rgba(0,0,0,0.5)] ${svgColor}`}>
          <svg viewBox="0 0 25.29 76.92" xmlns="http://www.w3.org/2000/svg" fill="currentColor" style={{shapeRendering: 'geometricPrecision', fillRule: 'evenodd', clipRule: 'evenodd'}}>
    ...
    return (
      <button ref={ref as React.Ref<HTMLButtonElement>} className={finalClassName} {...props}>
        {innerContent}
      </button>
    );
  }
