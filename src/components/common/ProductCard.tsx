...
export const ProductCard: React.FC<ProductCardProps> = ({ product, onQuickView }) => {
  ...
  return (
    <article className="group relative flex h-full w-full min-w-0 flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_6px_24px_rgba(15,23,42,0.06)] transition-all duration-300 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_14px_34px_rgba(15,23,42,0.12)]">
      <div className="relative flex h-52 w-full shrink-0 items-center justify-center overflow-hidden border-b border-slate-100 bg-white sm:h-56">
        <Link
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="mt-2 flex min-h-7 flex-wrap items-baseline gap-x-2 gap-y-0.5">
            <span className="whitespace-nowrap font-heading text-xl font-black leading-7 text-rose-500">
              {pricePrefix}{formatPrice(displayPrice, settings.currency)}
            </span>
            {displayOriginalPrice && displayOriginalPrice > displayPrice && !pricePrefix && (
              <span className="whitespace-nowrap text-xs font-semibold text-slate-400 line-through">
          ...
          <p className="mt-2 line-clamp-2 min-h-10 text-xs leading-5 text-slate-500">
            {cardDescription || 'A fun, quality toy selected for curious young minds.'}
          </p>
        ...
        <div className="mt-3 flex min-h-12 items-end gap-2 border-t border-slate-100 pt-3">
            <a 
              ...
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#25D366]/10 text-[#25D366] shadow-sm transition-colors hover:bg-[#25D366] hover:text-white"
            ...
            <button
              ...
              className={`relative flex h-10 min-w-0 flex-1 items-center justify-center gap-1.5 rounded-xl px-3 font-heading text-xs font-bold shadow-sm transition-all duration-300 active:scale-95 ${
                added
