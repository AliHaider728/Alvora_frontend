...
export const AdminStoreAppearancePageClient: React.FC = () => {
  ...
  if (!allowed) return <div className="max-w-xl rounded-3xl border border-amber-200 bg-amber-50 p-8"><ShieldAlert className="mb-3 h-8 w-8 text-[#C48B80]" /><h1 className="font-sans text-xl font-black text-[#1A1A1A]">Super Admin access required</h1><p className="mt-2 text-sm text-[#1A1A1A]/70">Only a Super Admin can change storefront navigation and homepage appearance.</p></div>;
  if (loading) return <div className="flex min-h-[40vh] items-center justify-center rounded-3xl border bg-white text-sm font-bold text-[#1A1A1A]/50">Loading storefront appearance…</div>;

  return <div className="space-y-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="font-sans text-2xl font-black text-[#1A1A1A]">Store Appearance</h1><p className="text-xs font-medium text-[#1A1A1A]/50">Build database-driven navigation and organize homepage content.</p></div><div className="flex flex-wrap gap-2">{tab === 'navigation' && <button type="button" onClick={addParent} className="flex items-center gap-2 rounded-xl border border-[#E7D9D0] bg-[#FAF6F2] px-4 py-2.5 text-xs font-bold text-[#C48B80]"><Plus className="h-4 w-4" /> Add Navigation Item</button>}<button type="button" onClick={() => void reset()} disabled={saving} className="flex items-center gap-2 rounded-xl border bg-white px-4 py-2.5 text-xs font-bold text-[#1A1A1A]/80"><RotateCcw className="h-4 w-4" /> Reset defaults</button><button type="button" onClick={() => void save()} disabled={saving || !dirty} className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#9C4122] to-[#B34E28] hover:from-[#9C4122] hover:to-[#7A321A] text-white border-transparent shadow-sm px-5 py-2.5 text-xs font-bold disabled:opacity-50"><Save className="h-4 w-4" />{saving ? 'Saving…' : 'Save appearance'}</button></div></div>
    <div className="flex w-fit rounded-2xl bg-slate-200/70 p-1">{(['navigation', 'homepage'] as Tab[]).map(value => <button key={value} type="button" onClick={() => setTab(value)} className={`flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold ${tab === value ? 'bg-white shadow-sm' : 'text-[#1A1A1A]/50'}`}>{value === 'navigation' ? <Menu className="h-4 w-4" /> : <Home className="h-4 w-4" />}{value === 'navigation' ? 'Navigation' : 'Homepage'}</button>)}</div>
...
const NavigationEditor: React.FC<EditorProps> = ({ item, categories, siblings, patchItem, move, changeMenuType, duplicate, remove }) => {
  ...
  const field = 'mt-1 w-full rounded-xl border border-[#E7D9D0] px-3 py-2 text-sm';
  return <div className="space-y-4"><div className="flex flex-wrap items-center gap-2"><div className="flex gap-1"><button type="button" disabled={index <= 0} onClick={() => move(item, -1)} aria-label={`Move ${item.label} up`} className="rounded-lg border p-2 disabled:opacity-30"><ArrowUp className="h-3.5 w-3.5" /></button><button type="button" disabled={index === ordered.length - 1} onClick={() => move(item, 1)} aria-label={`Move ${item.label} down`} className="rounded-lg border p-2 disabled:opacity-30"><ArrowDown className="h-3.5 w-3.5" /></button></div><span className="mr-auto rounded-full bg-[#FAF6F2] px-2.5 py-1 text-[10px] font-black text-[#1A1A1A]/50">{item.isSystemItem ? 'SYSTEM' : item.parentId ? 'CHILD' : 'CUSTOM'}</span>{!item.isSystemItem && <button type="button" onClick={() => duplicate(item)} className="rounded-lg p-2 text-[#1A1A1A]/50 hover:bg-[#FAF6F2]" aria-label={`Duplicate ${item.label}`}><Copy className="h-4 w-4" /></button>}{!item.isSystemItem && <button type="button" onClick={() => void remove(item)} className="rounded-lg p-2 text-[#A86249] hover:bg-[#FAF6F2]" aria-label={`Delete ${item.label}`}><Trash2 className="h-4 w-4" /></button>}</div>
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4"><label className="text-[11px] font-bold text-[#1A1A1A]/70">Label<input value={item.label} maxLength={60} onChange={e => patchItem(item.id, { label: e.target.value })} className={field} /></label>{!item.parentId && <label className="text-[11px] font-bold text-[#1A1A1A]/70">Menu Type<select value={item.menuType} onChange={e => changeMenuType?.(item, e.target.value as 'link' | 'dropdown')} className={field}><option value="link">Link</option><option value="dropdown">Dropdown</option></select></label>}{item.menuType === 'link' && <><label className="text-[11px] font-bold text-[#1A1A1A]/70">Link Type<select value={item.linkType} onChange={e => patchItem(item.id, { linkType: e.target.value as NavigationLinkType, path: undefined, externalUrl: undefined, categoryId: undefined })} className={field}><option value="internal_page">Internal Page</option><option value="category">Category</option><option value="custom_internal_url">Custom Internal URL</option><option value="external_url">External HTTPS URL</option></select></label><Destination item={item} categories={categories} patchItem={patchItem} field={field} /></>}<label className="text-[11px] font-bold text-[#1A1A1A]/70">Badge (optional)<input value={item.badgeText || ''} maxLength={20} onChange={e => patchItem(item.id, { badgeText: e.target.value })} className={field} placeholder="New" /></label></div>
    <div className="flex flex-wrap gap-4">{(['visible', 'enabled', 'showOnDesktop', 'showOnMobile'] as const).map(key => <label key={key} className="flex items-center gap-2 text-xs font-semibold text-[#1A1A1A]/70"><input type="checkbox" checked={item[key]} onChange={e => patchItem(item.id, { [key]: e.target.checked })} />{{ visible: 'Visible', enabled: 'Enabled', showOnDesktop: 'Desktop', showOnMobile: 'Mobile' }[key]}</label>)}{item.linkType === 'external_url' && item.menuType === 'link' && <label className="flex items-center gap-2 text-xs font-semibold text-[#1A1A1A]/70"><input type="checkbox" checked={item.openInNewTab === true} onChange={e => patchItem(item.id, { openInNewTab: e.target.checked })} /> Open in new tab</label>}</div>
  </div>;
};
const Destination: React.FC<{ item: StorefrontNavigationItem; categories: ReturnType<typeof useStore>['categories']; patchItem: EditorProps['patchItem']; field: string }> = ({ item, categories, patchItem, field }) => item.linkType === 'internal_page' ? <label className="text-[11px] font-bold text-[#1A1A1A]/70">Page<select value={item.path || '/'} onChange={e => patchItem(item.id, { path: e.target.value })} className={field}>{INTERNAL_PAGE_OPTIONS.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label> : item.linkType === 'category' ? <label className="text-[11px] font-bold text-[#1A1A1A]/70">Category<select value={item.categoryId || ''} onChange={e => patchItem(item.id, { categoryId: e.target.value })} className={field}><option value="">Choose category</option>{categories.filter(category => category.isActive !== false).map(category => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label> : item.linkType === 'external_url' ? <label className="text-[11px] font-bold text-[#1A1A1A]/70">HTTPS URL<input value={item.externalUrl || ''} onChange={e => patchItem(item.id, { externalUrl: e.target.value })} className={field} placeholder="https://example.com" /></label> : <label className="text-[11px] font-bold text-[#1A1A1A]/70">Public Path<input value={item.path || ''} onChange={e => patchItem(item.id, { path: e.target.value })} className={field} placeholder="/collections/new" /></label>;
const AppearanceField: React.FC<{ label: string; value: string; maxLength: number; onChange: (value: string) => void }> = ({ label, value, maxLength, onChange }) => <label className="text-[11px] font-bold text-[#1A1A1A]/70">{label}<input value={value} maxLength={maxLength} onChange={e => onChange(e.target.value)} className="mt-1 w-full rounded-xl border border-[#E7D9D0] px-3 py-2 text-sm" /><span className="mt-1 block text-right text-[10px] text-[#1A1A1A]/40">{value.length}/{maxLength}</span></label>;
