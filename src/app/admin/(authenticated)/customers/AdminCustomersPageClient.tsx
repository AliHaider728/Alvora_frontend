...
export const AdminCustomersPageClient: React.FC = () => {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-[#1A1A1A]/80">
            ...
            <tbody className="divide-y divide-slate-100">
              {customers.map(cust => (
                <tr key={cust.id} className="hover:bg-[#FAF6F2]/80 transition-colors">
                  ...
                  <td className="p-4 space-y-0.5">
                    <span className="flex items-center gap-1 font-medium text-[#1A1A1A]">
                      <Mail className="w-3 h-3 text-[#1A1A1A]/40" /> {cust.email}
                    </span>
                    <span className="flex items-center gap-1 text-[11px] text-[#1A1A1A]/50">
                      <Phone className="w-3 h-3 text-[#1A1A1A]/40" /> {cust.phone}
                    </span>
                  </td>
