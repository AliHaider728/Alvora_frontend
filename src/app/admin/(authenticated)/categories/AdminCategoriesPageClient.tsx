...
export const AdminCategoriesPageClient: React.FC = () => {
  ...
  if (!allowed) return <div className="max-w-xl rounded-3xl border border-amber-200 bg-amber-50 p-8"><ShieldAlert className="mb-3 h-8 w-8 text-[#C48B80]" /><h1 className="font-heading text-xl font-black text-[#1A1A1A]">Super Admin access required</h1><p className="mt-2 text-sm text-[#1A1A1A]/70">Normal Admins can assign existing categories from the product editor, but cannot change category structure.</p></div>;

  return <div className="space-y-6 font-heading">
    
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="font-heading text-2xl font-black text-[#1A1A1A]">Categories</h1><p className="text-xs font-medium text-[#1A1A1A]/50">Database-driven collections used across products, filters, homepage and navigation.</p></div><button type="button" onClick={() => setEditing(null)} className="flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#9C4122] to-[#B34E28] hover:from-[#9C4122] hover:to-[#7A321A] text-white border-transparent shadow-sm px-5 py-2.5 text-xs font-bold shadow-md"><Plus className="h-4 w-4" /> Add Category</button></div>
    ...
    {visibleCategories.length ? <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{visibleCategories.map((category, index) => <article key={category.id} className="rounded-3xl border border-[#E7D9D0] bg-white p-5 shadow-xs">
      <div className="flex gap-4">{category.image ? <img src={getSafeImageSrc(category.image)} alt="" className="h-16 w-16 shrink-0 rounded-2xl object-cover" /> : <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-[#FAF6F2] text-slate-300"><ImageIcon /></div>}<div className="min-w-0 flex-1"><h2 className="truncate font-heading text-base font-black text-[#1A1A1A]">{category.name}</h2><p className="truncate text-[11px] text-[#1A1A1A]/40">/{category.slug}</p><p className="mt-1 text-xs font-bold text-[#C48B80]">{category.itemCount || 0} products</p></div><div className="flex flex-col gap-1"><button type="button" disabled={index === 0} onClick={() => void move(category, -1)} aria-label={`Move ${category.name} up`} className="rounded-lg border p-1.5 disabled:opacity-30"><ArrowUp className="h-3.5 w-3.5" /></button><button type="button" disabled={index === visibleCategories.length - 1} onClick={() => void move(category, 1)} aria-label={`Move ${category.name} down`} className="rounded-lg border p-1.5 disabled:opacity-30"><ArrowDown className="h-3.5 w-3.5" /></button></div></div>
      <p className="mt-4 min-h-10 text-xs leading-5 text-[#1A1A1A]/70 line-clamp-2">{category.shortDescription || category.description || 'No description added.'}</p>
      <div className="mt-4 flex flex-wrap gap-2">{([['isActive', category.isActive !== false, 'Active', 'Hidden'], ['isFeatured', category.isFeatured === true, 'Featured', 'Not Featured'], ['showInNavigation', category.showInNavigation !== false, 'In Nav', 'Off Nav']] as const).map(([field, checked, yes, no]) => <button key={field} type="button" onClick={() => void toggle(category, field)} className={`rounded-full px-2.5 py-1 text-[10px] font-black ${checked ? 'bg-emerald-100 text-emerald-700' : 'bg-[#FAF6F2] text-[#1A1A1A]/50'}`}>{checked ? yes : no}</button>)}</div>
      <div className="mt-4 flex justify-end gap-2 border-t border-[#E7D9D0] pt-3"><button type="button" onClick={() => setEditing(category)} aria-label={`Edit ${category.name}`} className="rounded-xl p-2 text-[#C48B80] hover:bg-gradient-to-r hover:from-[#9C4122] hover:to-[#B34E28] hover:text-white hover:border-transparent hover:shadow-sm"><Edit2 className="h-4 w-4" /></button><button type="button" disabled={checkingDelete === category.id} onClick={() => void requestDelete(category)} aria-label={`Delete ${category.name}`} className="rounded-xl p-2 text-[#A86249] hover:bg-[#FAF6F2] disabled:opacity-50">{checkingDelete === category.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}</button></div>
    </article>)}</div> : <div className="rounded-3xl border border-dashed border-[#E7D9D0] bg-white p-10 text-center text-sm font-semibold text-[#1A1A1A]/50">No categories match this view.</div>}
    ...
    {deleteState && <div className="fixed inset-0 z-[108] flex items-center justify-center overflow-y-auto bg-slate-950/60 p-4" onMouseDown={e => { if (e.target === e.currentTarget && !deleting) setDeleteState(null); }}><div role="dialog" aria-modal="true" aria-labelledby="delete-category-title" className="my-auto w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
      <div className="flex items-start justify-between gap-4"><div><h2 id="delete-category-title" className="font-heading text-lg font-black text-[#1A1A1A]">Delete this category?</h2><p className="mt-2 text-sm leading-6 text-[#1A1A1A]/70"><strong>{deleteState.category.name}</strong> has {deleteState.productCount} affected product(s) and {deleteState.navigationCount} affected navigation item(s). Products will never be deleted.</p></div><button type="button" disabled={deleting} onClick={() => setDeleteState(null)} aria-label="Close" className="p-1 text-[#1A1A1A]/40"><X className="h-5 w-5" /></button></div>
      {deleteState.productCount > 0 && <fieldset className="mt-5 space-y-2"><legend className="text-xs font-black text-[#1A1A1A]/80">Products</legend><label className="flex gap-2 rounded-xl border p-3 text-xs font-semibold"><input type="radio" checked={productResolution === 'uncategorized'} onChange={() => setProductResolution('uncategorized')} /> Move products to Uncategorized</label><label className="flex gap-2 rounded-xl border p-3 text-xs font-semibold"><input type="radio" checked={productResolution === 'reassign'} onChange={() => setProductResolution('reassign')} /> Move products to another category</label></fieldset>}
      {deleteState.navigationCount > 0 && <fieldset className="mt-5 space-y-2"><legend className="text-xs font-black text-[#1A1A1A]/80">Navigation links ({deleteState.navigationItems.map(item => item.label).join(', ')})</legend><label className="flex gap-2 rounded-xl border p-3 text-xs font-semibold"><input type="radio" checked={navigationResolution === 'remove'} onChange={() => setNavigationResolution('remove')} /> Remove affected navigation links</label><label className="flex gap-2 rounded-xl border p-3 text-xs font-semibold"><input type="radio" checked={navigationResolution === 'reassign'} onChange={() => setNavigationResolution('reassign')} /> Reassign links to another category</label></fieldset>}
      {(productResolution === 'reassign' || navigationResolution === 'reassign') && <label className="mt-4 block text-xs font-black text-[#1A1A1A]/80">Replacement Category<select value={targetCategoryId} onChange={e => setTargetCategoryId(e.target.value)} className="mt-1.5 w-full rounded-xl border border-[#E7D9D0] px-3 py-2.5 text-sm"><option value="">Choose a category</option>{categories.filter(item => item.id !== deleteState.category.id).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}
      <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" disabled={deleting} onClick={() => setDeleteState(null)} className="rounded-xl bg-[#FAF6F2] px-4 py-2.5 text-xs font-bold text-[#1A1A1A]/80">Cancel</button><button type="button" disabled={deleting} onClick={() => void confirmDelete()} className="flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-[#9C4122] to-[#B34E28] hover:from-[#9C4122] hover:to-[#7A321A] text-white border-transparent shadow-sm px-5 py-2.5 text-xs font-bold disabled:opacity-50">{deleting && <Loader2 className="h-4 w-4 animate-spin" />}{productResolution === 'reassign' ? 'Reassign and Delete' : 'Move to Uncategorized and Delete'}</button></div>
    </div></div>}
  </div>;
};
