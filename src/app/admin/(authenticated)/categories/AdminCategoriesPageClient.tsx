...
export const AdminCategoriesPageClient: React.FC = () => {
  ...
  if (!allowed) return <div className="max-w-xl rounded-3xl border border-amber-200 bg-amber-50 p-8"><ShieldAlert className="mb-3 h-8 w-8 text-[#C48B80]" /><h1 className="font-sans text-xl font-black text-[#1A1A1A]">Super Admin access required</h1><p className="mt-2 text-sm text-[#1A1A1A]/70">Normal Admins can assign existing categories from the product editor, but cannot change category structure.</p></div>;

  return <div className="space-y-6 font-sans">
    
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="font-sans text-2xl font-black text-[#1A1A1A]">Categories</h1><p className="text-xs font-medium text-[#1A1A1A]/50">Database-driven collections used across products, filters, homepage and navigation.</p></div><button type="button" onClick={() => setEditing(null)} className="flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#9C4122] to-[#B34E28] hover:from-[#9C4122] hover:to-[#7A321A] text-white border-transparent shadow-sm px-5 py-2.5 text-xs font-bold shadow-md"><Plus className="h-4 w-4" /> Add Category</button></div>
    ...
    {deleteState && <div className="fixed inset-0 z-[108] flex items-center justify-center overflow-y-auto bg-slate-950/60 p-4" onMouseDown={e => { if (e.target === e.currentTarget && !deleting) setDeleteState(null); }}><div role="dialog" aria-modal="true" aria-labelledby="delete-category-title" className="my-auto w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
      <div className="flex items-start justify-between gap-4"><div><h2 id="delete-category-title" className="font-sans text-lg font-black text-[#1A1A1A]">Delete this category?</h2><p className="mt-2 text-sm leading-6 text-[#1A1A1A]/70"><strong>{deleteState.category.name}</strong> has {deleteState.productCount} affected product(s) and {deleteState.navigationCount} affected navigation item(s). Products will never be deleted.</p></div><button type="button" disabled={deleting} onClick={() => setDeleteState(null)} aria-label="Close" className="p-1 text-[#1A1A1A]/40"><X className="h-5 w-5" /></button></div>
