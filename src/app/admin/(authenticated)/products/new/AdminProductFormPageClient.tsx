...
const FieldError: React.FC<{ message?: string }> = ({ message }) =>
  message ? <p role="alert" className="mt-1.5 text-xs font-semibold text-[#A86249]">{message}</p> : null;
        ...
        <div className="min-w-0 space-y-6">
          <FormCard title="Basic Information" icon={Info}>
            <div className="grid gap-5 sm:grid-cols-2">
              <label className="sm:col-span-2">
                <span className="mb-1.5 block text-xs font-bold text-[#1A1A1A]/80">Product Type</span>
              ...
              <label className="sm:col-span-2">
                <span className="mb-1.5 block text-xs font-bold text-[#1A1A1A]/80">Product Name <span className="text-[#C48B80]">*</span></span>
              ...
              <label>
                <span className="mb-1.5 block text-xs font-bold text-[#1A1A1A]/80">Short Description</span>
                ...
                <span className="mt-1 block text-right text-[10px] text-[#1A1A1A]/40">{shortDescription.length}/300</span>
              </label>
              <fieldset className="sm:col-span-2">
                ...
                <div className="rounded-2xl border border-[#E7D9D0] bg-[#FAF6F2]/70 p-3">
                ...
                <p className="mt-1.5 text-[10px] text-[#1A1A1A]/40">The first selected category is used as the primary category on cards and breadcrumbs.</p>
          ...
          {productType === 'simple' && (
            <FormCard title="Pricing & Stock" icon={CircleDollarSign}>
              <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                <label>
                  <span className="mb-1.5 block text-xs font-bold text-[#1A1A1A]/80">Regular Price (Rs.) <span className="text-[#C48B80]">*</span></span>
                ...
                <div className="flex items-center justify-between gap-4 rounded-xl border border-[#E7D9D0] px-3 py-2.5">
                  ...
                  <Switch checked={trackInventory} onChange={(val) => { setTrackInventory(val); markDirty(); clearError("stockQuantity"); }} />
                </div>
                {trackInventory ? <>
                  <label>
                    <span className="mb-1.5 block text-xs font-bold text-[#1A1A1A]/80">Stock Quantity <span className="text-[#C48B80]">*</span></span>
                  ...
                  <span className="mb-1.5 block text-xs font-bold text-[#1A1A1A]/80">Manual Stock Status</span>
            ...
            <>
              ...
              {attributes.some(a => a.slug === 'size') && (
                <FormCard title="Size Guide" description="Add a size chart or measurements. If provided, a 'Size Guide' button will appear on the product page." icon={Info}>
                  <div className="overflow-hidden rounded-xl border border-[#E7D9D0]">
                    <textarea 
                      value={sizeGuide} 
              ...
              <FormCard title="Variations" description="Generate and manage product variations." icon={PackageCheck}>
                ...
                {variations.length > 0 && (
                  <div className="mt-6 p-4 bg-[#FAF6F2] border border-[#E7D9D0] rounded-xl">
                    <label className="block text-sm font-bold text-[#1A1A1A] mb-2">Default Variation</label>
                    <p className="text-xs text-[#1A1A1A]/50 mb-3">This variation will be pre-selected when customers open the product page.</p>
                    <select
                    ...
                    >
                      ...
                      {enabledVariations.map((v, i) => (
                        <option key={String(v.id)} value={String(v.id)} className="text-[#1A1A1A]">
          ...
          <FormCard title="Delivery & Shipping" icon={Truck}>
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              <label>
                <span className="mb-1.5 block text-xs font-bold text-[#1A1A1A]/80">Delivery Charge Model</span>
                ...
                <span className="mt-1.5 block text-[10px] text-[#1A1A1A]/40">Product overrides take priority over the store-wide threshold.</span>
              </label>
              {deliveryType === 'fixed' && <label className="sm:col-span-2"><span className="mb-1.5 block text-xs font-bold text-[#1A1A1A]/80">Custom Shipping Fee (Rs.) <span className="text-[#C48B80]">*</span></span><input type="number" min="0" step="1" value={customDeliveryFee ?? ''} onChange={event => { setCustomDeliveryFee(event.target.value === '' ? undefined : Number(event.target.value)); markDirty(); clearError('customDeliveryFee'); }} className={inputClass('customDeliveryFee')} /><FieldError message={errors.customDeliveryFee} /></label>}
            </div>
          </FormCard>
        ...
        <aside className="min-w-0 space-y-6 xl:sticky xl:top-20">
          <FormCard title="Publish" icon={Send}>
            <div className="space-y-4">
              <label><span className="mb-1.5 block text-xs font-bold text-[#1A1A1A]/80">Status</span><select value={status} onChange={event => { setStatus(event.target.value as 'draft' | 'published'); markDirty(); }} className={fieldClassName}><option value="published">Published</option><option value="draft">Draft</option></select></label>
              ...
              <div className="flex items-center justify-between gap-4 border-t border-[#E7D9D0] pt-4">
                ...
                <Switch checked={isFeatured} onChange={(val) => { setIsFeatured(val); markDirty(); }} />
              </div>
              {[
                ...
                <div key={option.label} className="flex items-center justify-between gap-4 border-t border-[#E7D9D0] pt-4">
                  ...
                  <Switch checked={option.value} onChange={(val) => { option.setValue(!option.value); markDirty(); }} aria-label={option.label} />
                </div>
              ))}
          ...
          <FormCard title="Product Images" icon={ImagePlus}>
            <div className="space-y-5">
              <div>
                <span className="mb-2 block text-xs font-bold text-[#1A1A1A]/80">Main Product Image <span className="text-[#C48B80]">*</span></span>
                {images[0] ? (
                  ...
                  <label className={`flex aspect-[4/3] cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed bg-[#FAF6F2] text-[#1A1A1A]/70 group text-center transition hover:border-[#C48B80] hover:bg-gradient-to-r hover:from-[#9C4122] hover:to-[#B34E28] hover:text-white hover:border-transparent hover:shadow-sm/40 ${errors.images ? 'border-[#C48B80]' : 'border-[#E7D9D0]'}`}><input type="file" className="hidden" accept="image/jpeg,image/png,image/webp" disabled={uploadingTarget !== null} onChange={event => { void uploadImages(Array.from(event.target.files || []), 'main'); event.target.value = ''; }} />{uploadingTarget === 'main' ? <Loader2 className="h-7 w-7 animate-spin text-[#C48B80]" /> : <ImageIcon className="h-7 w-7 text-[#C48B80] group-hover:text-white transition-colors" />}<span className="mt-2 text-xs font-bold">Upload main image</span><span className="mt-1 text-[10px] opacity-70">JPG, PNG, WebP &bull; Max 5MB</span></label>
                )}
              ...
              <div>
                ...
                {galleryImages.length > 0 && <div className="mb-3 grid grid-cols-2 gap-2">{galleryImages.map((image, galleryIndex) => { const imageIndex = galleryIndex + 1; return <div key={image.id} className="overflow-hidden rounded-xl border border-[#E7D9D0] bg-[#FAF6F2]"><img src={getSafeImageSrc(image.url)} alt={`Gallery preview ${imageIndex}`} className="aspect-square w-full object-cover" /><div className="flex items-center justify-between gap-1 p-1.5"><button type="button" title="Make main image" onClick={() => makeMainImage(imageIndex)} className="rounded-md px-1.5 py-1 text-[9px] font-bold text-[#C48B80] hover:bg-gradient-to-r hover:from-[#9C4122] hover:to-[#B34E28] hover:text-white hover:border-transparent hover:shadow-sm">Main</button><button type="button" disabled={galleryIndex === 0} onClick={() => moveGalleryImage(imageIndex, -1)} className="rounded-md p-1 text-[#1A1A1A]/50 disabled:opacity-30" aria-label="Move image left"><ArrowLeft className="h-3 w-3" /></button><button type="button" disabled={galleryIndex === galleryImages.length - 1} onClick={() => moveGalleryImage(imageIndex, 1)} className="rounded-md p-1 text-[#1A1A1A]/50 disabled:opacity-30" aria-label="Move image right"><ArrowRight className="h-3 w-3" /></button><button type="button" onClick={() => { void removeImage(image.id); }} className="rounded-md p-1 text-[#C48B80]" aria-label="Remove gallery image"><Trash2 className="h-3 w-3" /></button></div></div>; })}</div>}
                <label className={`flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-[#E7D9D0] px-3 py-3 text-xs font-bold text-[#1A1A1A]/70 group hover:border-[#C48B80] hover:bg-gradient-to-r hover:from-[#9C4122] hover:to-[#B34E28] hover:text-white hover:border-transparent hover:shadow-sm/40 ${galleryImages.length >= 8 ? 'pointer-events-none opacity-50' : ''}`}>{uploadingTarget === 'gallery' ? <Loader2 className="h-4 w-4 animate-spin text-[#C48B80]" /> : <ImagePlus className="h-4 w-4 text-[#C48B80] group-hover:text-white transition-colors" />} Add Gallery Images<input type="file" multiple className="hidden" accept="image/jpeg,image/png,image/webp" disabled={uploadingTarget !== null || galleryImages.length >= 8} onChange={event => { void uploadImages(Array.from(event.target.files || []), 'gallery'); event.target.value = ''; }} /></label>
              </div>
            </div>
          </FormCard>
          ...
          <FormCard title="Product Details" icon={Box}>
            <div className="space-y-4">
              ...
              <label><span className="mb-1.5 block text-xs font-bold text-[#1A1A1A]/80">Material</span><input value={material} onChange={event => { setMaterial(event.target.value); markDirty(); }} className={fieldClassName} placeholder="e.g. ABS Plastic (BPA Free)" /></label>
              <label><span className="mb-1.5 block text-xs font-bold text-[#1A1A1A]/80">Safety Notes</span><textarea rows={3} value={safetyInfo} onChange={event => { setSafetyInfo(event.target.value); markDirty(); }} className={fieldClassName} placeholder="Child-safe materials and relevant warnings" /></label>
            </div>
          </FormCard>
