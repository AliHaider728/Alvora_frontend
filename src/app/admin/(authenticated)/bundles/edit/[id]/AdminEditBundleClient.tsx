...
export default function AdminEditBundleClient({ bundleId }: { bundleId: string }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const fetchedProducts = Array.isArray(productsRes)
        ? productsRes
  ...
  const currentSaleTotal = useMemo(() => {
    return selectedProducts.reduce((sum, sp) => {
      const price = Number(sp.product.price) || 0;
  ...
  const calculatedStock = useMemo(() => {
    ...
    const stocks = selectedProducts.map(sp => sp.product.trackInventory === false ? Infinity : Math.floor((sp.product.stockQuantity || 0) / sp.qty));
                    ...
                    <label className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-[#A85A3B] hover:bg-gray-50 transition-colors bg-white">
                      {uploadingImage ? (
                        ...
                        <div className="text-center space-y-2">
                          <div className="w-12 h-12 rounded-full bg-orange-50 text-[#A85A3B] flex items-center justify-center mx-auto mb-3">
                            <Upload className="w-5 h-5" />
                          </div>
                          <p className="text-sm font-semibold text-gray-900">Click to upload custom image</p>
