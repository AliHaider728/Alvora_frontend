...
export const CategoryPageClient: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FAF6F2] font-sans pt-6 pb-32">
      ...
      <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Page Header */}
        <div className="relative w-full min-h-[250px] md:min-h-[300px] lg:min-h-[350px] py-10 mb-8 bg-[#E9E1D9] overflow-hidden rounded-xl flex items-center">
          ...
          <div className="relative z-20 px-8 md:px-16 max-w-2xl">
            ...
            <h1 className="font-display text-3xl sm:text-4xl lg:text-5xl text-[#1A1A1A] font-medium leading-tight mb-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <aside className="hidden lg:block space-y-6 bg-white p-6 rounded-2xl border border-[#E7D9D0] shadow-sm h-fit sticky top-24">
            <div className="flex items-center justify-between pb-4 border-b border-[#EDE5DC]">
              <div className="flex items-center gap-2 font-display font-extrabold text-[#1A1A1A] text-base">
                <Filter className="w-5 h-5 text-[#9C4122]" />
                <span>Filter Products</span>
              </div>
              <button
                onClick={resetFilters}
                className="text-xs font-bold text-[#9C4122] hover:text-[#7A321A] flex items-center gap-1"
              >
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1">
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-full text-xs font-bold transition-colors ${
                ...
                {categories.map(cat => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-full text-xs font-semibold transition-colors flex items-center justify-between ${
              ...
              <div className="space-y-1">
                {[0, 4, 4.5, 4.8].map(r => (
                  <button
                    ...
                    className={`w-full flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${
          ...
          <main className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-white p-4 rounded-2xl border border-[#EDE5DC] shadow-sm flex flex-wrap items-center justify-between gap-4">
              <button
                onClick={() => setMobileFilterOpen(true)}
                className="btn-interactive lg:hidden flex items-center gap-2 px-4 py-2 rounded-2xl bg-gradient-to-br from-[#D4784F] to-[#9C4122] text-white font-display font-medium uppercase tracking-widest text-xs"
              >
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="px-3.5 py-2 text-xs font-display font-medium uppercase tracking-widest rounded-2xl border border-[#EDE5DC] bg-[#FAF6F2] text-[#1A1A1A] focus:outline-none focus:ring-2 focus:ring-[#C48B80] cursor-pointer"
                >
            ...
            {productsLoading ? (
              ...
              <div className="bg-white rounded-2xl p-12 text-center border border-[#EDE5DC] space-y-4">
        ...
        <div className="fixed inset-0 z-50 overflow-hidden lg:hidden">
          ...
          <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
            <div className="w-screen max-w-xs bg-white shadow-2xl p-6 pb-12 overflow-y-auto space-y-6">
              ...
              {/* Mobile Categories */}
              <div className="space-y-2">
                <h4 className="font-display font-medium uppercase tracking-widest text-xs uppercase text-[#1A1A1A]/60">Category</h4>
                <div className="space-y-1">
                  <button
                    onClick={selectAllCategories}
                    aria-pressed={selectedCategories.length === 0}
                    className={`w-full text-left px-3 py-2 rounded-full text-xs font-bold ${selectedCategories.length === 0 ? 'bg-gradient-to-br from-[#D4784F] to-[#9C4122] text-white' : 'text-[#241916]/80'}`}
                  >
                    All Categories
                  </button>
                  {categories.map(cat => (
                    <button
                      ...
                      onClick={() => toggleCategory(cat.slug)}
                      aria-pressed={selectedCategories.includes(cat.slug)}
                      className={`w-full text-left px-3 py-2 rounded-full text-xs font-semibold ${selectedCategories.includes(cat.slug) ? 'bg-gradient-to-br from-[#D4784F] to-[#9C4122] text-white font-bold' : 'text-[#241916]/80'}`}
                    >
                      {cat.name}
                    </button>
                  ))}
                ...
                <div className="space-y-1">
                  {[0, 4, 4.5, 4.8].map(r => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-full text-xs font-semibold ${
                        minRating === r
                          ? 'bg-gradient-to-br from-[#D4784F] to-[#9C4122] text-white font-bold'
                          : 'text-[#241916]/80'
                      }`}
              ...
              <div className="grid grid-cols-2 gap-2 pt-4 border-t border-[#EDE5DC]">
                <button
                  onClick={resetFilters}
                  className="w-full py-3 rounded-xl bg-[#F5EDE4] text-[#241916]/80 font-display font-medium uppercase tracking-widest text-xs"
                >
                  Reset
                </button>
                <button
                  onClick={() => setMobileFilterOpen(false)}
                  className="w-full py-3 rounded-2xl bg-slate-900 text-white font-display font-medium uppercase tracking-widest text-xs"
                >
                  Apply & Close
                </button>
              </div>
