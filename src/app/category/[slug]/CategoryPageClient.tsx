                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-xl text-xs font-bold transition-colors ${
                    selectedCategories.length === 0
                      ? 'bg-gradient-to-r from-[#C48B80] to-[#E2A08C] text-white'
                  ...
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-xl text-xs font-semibold transition-colors flex items-center justify-between ${
                      selectedCategories.includes(cat.slug)
                        ? 'bg-gradient-to-r from-[#C48B80] to-[#E2A08C] text-white font-bold'
                        : 'text-[#241916]/80 hover:bg-[#F5EDE4]'
                    }`}
                  >
                    <span>{cat.name}</span>
          ...
          <main className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-white p-4 rounded-sm border border-[#EDE5DC] shadow-sm flex flex-wrap items-center justify-between gap-4">
              <button
                ...
                className="lg:hidden flex items-center gap-2 px-4 py-2 rounded-2xl bg-gradient-to-r from-[#C48B80] to-[#E2A08C] text-white font-display font-medium text-2xl uppercase tracking-widest text-xs"
              >
            ...
            {loading ? (
              ...
              <div className="bg-white rounded-sm p-12 text-center border border-[#EDE5DC] space-y-4">
                ...
                <button
                  onClick={resetFilters}
                  className="px-6 py-2.5 rounded-2xl bg-gradient-to-r from-[#C48B80] to-[#E2A08C] text-white font-display font-medium text-2xl uppercase tracking-widest text-xs hover:bg-[#A86249] transition-colors"
                >
                  Clear All Filters
                </button>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  <button
                    onClick={selectAllCategories}
                    aria-pressed={selectedCategories.length === 0}
                    className={`w-full text-left px-3 py-2 rounded-xl text-xs font-bold ${selectedCategories.length === 0 ? 'bg-gradient-to-r from-[#C48B80] to-[#E2A08C] text-white' : 'text-[#241916]/80'}`}
                  >
                    All Categories
                  </button>
                  {categories.map(cat => (
                    <button
                      ...
                      onClick={() => toggleCategory(cat.slug)}
                      aria-pressed={selectedCategories.includes(cat.slug)}
                      className={`w-full text-left px-3 py-2 rounded-xl text-xs font-semibold ${selectedCategories.includes(cat.slug) ? 'bg-gradient-to-r from-[#C48B80] to-[#E2A08C] text-white font-bold' : 'text-[#241916]/80'}`}
                    >
                      {cat.name}
                    </button>
                  ))}
              ...
              <div className="space-y-2 pt-4 border-t border-[#EDE5DC]">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={selectAllAges}
                    aria-pressed={selectedAges.length === 0}
                    className={`px-2 py-2 rounded-xl text-xs font-bold ${selectedAges.length === 0 ? 'bg-gradient-to-r from-[#C48B80] to-[#E2A08C] text-white' : 'bg-[#F5EDE4]'}`}
                  >
                    All Ages
                  </button>
                  {AGE_GROUPS.map(age => (
                    <button
                      ...
                      onClick={() => toggleAge(age.id)}
                      aria-pressed={selectedAges.includes(age.id)}
                      className={`px-2 py-2 rounded-xl text-xs font-bold ${selectedAges.includes(age.id) ? 'bg-gradient-to-r from-[#C48B80] to-[#E2A08C] text-white' : 'bg-[#F5EDE4]'}`}
                    >
                      {age.label}
                    </button>
                  ))}
