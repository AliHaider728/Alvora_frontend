...
export const api = {
  ...
  getBundle: async (slug: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        // Fallback to getting all bundles and filtering if slug endpoint doesn't exist
        const allRes = await fetch(`${API_BASE_URL}/bundles`, {
          cache: "no-store",
          headers: { "Accept": "application/json" }
        });
        if (!allRes.ok) throw new Error("Failed to fetch bundles");
        const data = await allRes.json();
        const bundles = data.bundles || data;
        return bundles.find((b: any) => b.slug === slug || b.id === slug) || null;
      }
      return res.json();
    } catch (error: any) {
